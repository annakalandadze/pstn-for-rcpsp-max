...
def write_graphml(filename, ncontingent, networktype, nedges, nvertices, name, edges, nodes):
    ...
    keys = [
        ...
        ("Value", "edge", "Value for PSTN edge. Format: 'integer'", " "),
    ...
    for node_id, x, y, log_normal in nodes:
        node = ET.SubElement(graph, "node", id=node_id)
        ET.SubElement(node, "data", key="LogNormalDistribution").text = log_normal
...
def pstn_to_xml(pstn, name_graph, directory):
    edges = []
    for node_from in pstn.nodes:
        for node_to in pstn.nodes:
            if node_to in pstn.edges[node_from]:
                edge = pstn.edges[node_from][node_to]
                node_from_str = pstn.translation_dict[node_from]
                node_to_str = pstn.translation_dict[node_to]
    ...
    for node in pstn.nodes:
        node_str = pstn.translation_dict[node]
        if node in pstn.probabilistic_durations and pstn.probabilistic_durations[node] is not None:
            mean = pstn.probabilistic_durations[node]["mean"]
            std = pstn.probabilistic_durations[node]["std"]
            log_normal_params = f"LogNormalDistributionParameter[location={mean}, scale={std}]"
        else:
            log_normal_params = ""

        nodes.append((node_str, str(random.randint(40, 200)), str(random.randint(40, 200)), log_normal_params))

    ncontingent = len(pstn.contingent_links)
    networktype = "PSTN"
    nedges = len(edges)
    nvertices = len(pstn.nodes)
    write_graphml(f"{directory}/{name_graph}.pstn", ncontingent, networktype, nedges, nvertices, name_graph, edges, nodes)
